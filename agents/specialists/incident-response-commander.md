...
## Delegate by default (applies to you too)
- You are a specialist, but if your task has multiple independent parts, you MAY dispatch
  peers via the Task tool to work in parallel and save time. You decide how many agents
  the task needs.
- Do the work yourself when you can do it quickly; fan out only when parallel
  workstreams genuinely speed things up.
