// Shared: merge the Agent-Teams `subagent_depth` default into the USER's global
...
// The native fork core understands it via root `subagent_depth` (V2 schema-compliant);
...
/**
 * Apply (agents mode) or remove (full mode) root `subagent_depth` in
...
export async function mergeSubagentDepth(configRoot, log = console.log, mode = "full") {
  ...
  if (mode === "agents") {
    ...
    // 1. Migrate misplaced experimental.subagent_depth back to root if present
    ...
    // 2. Ensure root subagent_depth exists
    if (config.subagent_depth === undefined) {
